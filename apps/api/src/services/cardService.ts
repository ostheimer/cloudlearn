...
import { z } from "zod";
import { createCard, softDeleteCard, updateCard } from "@/lib/inMemoryStore";
...
export function updateCardForUser(input: unknown) {
  ...
  if (parsed.front !== undefined) {
    updates.front = parsed.front;
  }
  if (parsed.back !== undefined) {
    updates.back = parsed.back;
  }
...
export function deleteCardForUser(cardId: string): boolean {
