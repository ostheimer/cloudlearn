...
export type AdState = "idle" | "loading" | "ready" | "showing" | "rewarded" | "failed" | "cap_reached";
...
// Falls back to a short simulation if the native SDK is unavailable (Expo Go, web).
async function loadAndShowRewardedAd(): Promise<boolean> {
  try {
    const { RewardedAd, RewardedAdEventType, TestIds } =
      await import("react-native-google-mobile-ads");

    const adUnitId = __DEV__ ? TestIds.REWARDED : (ADMOB_REWARDED_ID ?? TestIds.REWARDED);
    ...
    return await new Promise<boolean>((resolve) => {
      ...
      // ERROR event type varies by SDK version; use a catch-all listener
      ad.addAdEventListener("error" as Parameters<typeof ad.addAdEventListener>[0], () => {
        resolve(false);
      });
    ...
    // SDK not available (Expo Go, web) — fall back to simulation
...
export function useRewardedAd(): UseRewardedAdReturn {
  ...
  const watchAd = useCallback(async (): Promise<RewardedAdResult | null> => {
    ...
    try {
      ...
      // Grant LP server-side (tier-gated, cap-checked)
