...
import type { CardSource } from "../components/cardSourcePicker";

/**
 * Zuletzt gewählte Einstellungen eines Lernmodus, gemerkt je Deck und Modus
 * auf diesem Gerät (#610) — das Web-Gegenstück ist
 * apps/web/src/lib/setup-memory.ts (gleiche Schlüssel, gleiche Regeln). Ohne
 * den Merker begann jeder Einstell-Bildschirm wieder bei den Standardwerten;
 * wer z. B. immer „Rückseite zuerst" mit „Nur markierte" lernt, musste das
 * jedes Mal neu umstellen.
 *
...
export async function saveSetup(
  ...
  try {
    await AsyncStorage.setItem(storageKey(deckId, mode), JSON.stringify(setup));
  } catch {
    // Speicher kann klemmen (voll, im Abbau): Dann fehlt nur die
...
export async function loadSetup(
  deckId: string,
  mode: SetupMode
): Promise<StoredSetup | null> {
  try {
    return parseStoredSetup(await AsyncStorage.getItem(storageKey(deckId, mode)));
  } catch {
    return null;
  }
}
