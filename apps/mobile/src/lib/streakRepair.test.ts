...
describe("Streak-Reparatur nennt den Kontostand (#611)", () => {
  ...
  it("sperrt NICHT, solange der Kontostand unbekannt ist", () => {
    // Die eigentliche Falle: Der usageStore startet mit einer Vorbelegung von
    // 10 LP. Nähme man die für eine Auskunft, wäre der Knopf für JEDES Konto
    // grau, bis der echte Stand geladen ist — auch für eines mit 200 LP.
  ...
  it("nennt in der Nachfrage beide Zahlen", () => {
    expect(streakRepairPrompt(12, 40, 7)).toBe(
      "Das kostet 40 LP — du hast 12 LP. Damit kommt dein 7-Tage-Streak zurück."
    );
  });

  it("fällt in der Nachfrage auf den alten Wortlaut zurück", () => {
    expect(streakRepairPrompt(null, 40, 7)).toBe(
      "Das kostet 40 LP und stellt deinen 7-Tage-Streak wieder her."
    );
  });
});
