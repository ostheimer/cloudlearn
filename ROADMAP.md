...
Letzte Aktualisierung: 2026-07-23 (Mobile-LP-Abrechnung Bild-Abdecken)
...
- 2026-07-23: **Mobile-LP-Abrechnung Bild-Abdecken (Cron-Pass + Review-Fix)**: `occlusion.tsx` hatte nach #463/`sendReview` weiter `awardedRef`, wartete nicht auf laufende Reviews und behielt die veraltete lokale Fünf-Karten-Schwelle. Dadurch erschien die Gutschrift bei langsamer Verbindung oder Sitzungen mit 1–4 Karten nicht sofort; die Reviews blieben durch das Server-Wasserzeichen aber für eine spätere Gutschrift erhalten. Portiert `beginSessionAward` wie Web-Occlusion (#252) und gleicht die Client-Schwelle an die serverseitige Ein-Karten-Abrechnung an; Strukturtests sichern Reihenfolge und Mindestmenge.
