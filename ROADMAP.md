...
#### Fix: Guard AdMob-Konfiguration (#14)
- `EXPO_PUBLIC_ADMOB_*`-Variablen werden nur gelesen, wenn sie gesetzt sind; kein Crash mehr bei fehlendem AdMob-Setup in Dev/Preview.

#### Fix: Guard RevenueCat-Konfiguration (#15)
- `EXPO_PUBLIC_REVENUECAT_API_KEY` und Produkt-IDs werden erst zur Laufzeit geprüft; fehlende Keys in Dev/Preview führen zu einem gelogten Warn statt einem unbehandelten Fehler.

#### Fix: Release-Links angleichen (#16)
- Changelog- und Release-Links im README auf konsistentes Format gebracht.

#### Fix: LP-Pack-Käufe absichern (#17, #18)
- Guard für `purchaseLPPack`-Aufrufe: fehlende RevenueCat-Konfiguration bricht den Kauf-Flow nicht mehr ab, sondern gibt eine nutzerfreundliche Fehlermeldung.
