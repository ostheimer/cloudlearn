...
| D5 | CL-D05 | **Apple/Google Sign-In** (OAuth neben E-Mail) | Mittel | Mittel | 🔄 In Arbeit (Flow in `apps/mobile/src/lib/oauth.ts` implementiert; Supabase-Provider-Aktivierung und reale Tests ausstehend) |
...
- 2026-06-15: **CL-D05 Status-Abgleich**: Status von ❌ Offen auf 🔄 In Arbeit gesetzt — OAuth-Flow existiert bereits in `apps/mobile/src/lib/oauth.ts`; Supabase-Provider-Aktivierung und reale Tests stehen noch aus (gemäß BACKLOG.md).
