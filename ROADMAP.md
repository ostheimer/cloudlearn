...
- 2026-07-22: **Mobile-LP-Race Occlusion (Cron-Pass)**: `occlusion.tsx` hatte nach #463/`sendReview` weiter `awardedRef` ohne auf laufende Reviews zu warten — `earnLp` lief vor Server-Eingang, `granted: 0` wurde permanent. Portiert `beginSessionAward` wie Web-Occlusion (#252); Strukturtest in `learn-session-lp-screens.test.ts`.
